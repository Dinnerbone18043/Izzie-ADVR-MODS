...
function onPickupActivate()
	...
	if loadValue("NTS-Spawner-Mode") == 4 then
		game.spawnObjectNetwork("enemy_bc_skull_darkness", game.playerController.rightHand.transform.position + vector3.__new(0, 0.5, 0))
		return
	end
	...
	if loadValue("NTS-Spawner-Mode") == 26 then
		local enemy = game.spawnObjectNetwork("projectile_book_exploding", game.playerController.rightHand.transform.position)
		local gameObject = enemy.gameObject
		local Base = getComponent(gameObject, "BulletBase")
		Base.shotFromPlayer = true
		Base.destroyOnCollide = true
		Base.ignoreTriggerInteractions = false
		Base.rotateTowardsVelocity = true
		Base.Start()
		enemy.transform.SetParent(game.playerController.rightHand.transform)
		enemy.transform.position = game.playerController.rightHand.transform.position + vector3.__new(0, 0, 0.35)
		enemy.transform.rotation = game.playerController.rightHand.transform.rotation
		function Base.OnCollisionOrTrigger()
			destroy(gameObject)
		end
	...
	if loadValue("NTS-Spawner-Mode") == 27 then
		local orb = game.SpawnObjectNetwork("item_upgrade_all", game.playerController.rightHand.transform.position)
		local itembase = getComponent(orb, "ItemUpgrade")
		itembase.forceSpawn = "erthu_chrwa_vial_of_blue_blood"

		local orb = game.SpawnObjectNetwork("item_upgrade_all", game.playerController.rightHand.transform.position)
		local itembase = getComponent(orb, "ItemUpgrade")
		itembase.forceSpawn = "erthu_chrwa_slobbering_totem"
		return
	end

	if loadValue("NTS-Spawner-Mode") == 28 then
		game.spawnObjectNetwork("item_key", game.playerController.rightHand.transform.position)
		game.spawnObjectNetwork("item_key", game.playerController.rightHand.transform.position)
		game.spawnObjectNetwork("item_key", game.playerController.rightHand.transform.position)
		game.spawnObjectNetwork("item_key", game.playerController.rightHand.transform.position)
	...
	if loadValue("NTS-Spawner-Mode") == 29 then
		game.spawnObjectNetwork("item_coin", game.playerController.rightHand.transform.position)
		game.spawnObjectNetwork("item_coin", game.playerController.rightHand.transform.position)
		game.spawnObjectNetwork("item_coin", game.playerController.rightHand.transform.position)
		game.spawnObjectNetwork("item_coin", game.playerController.rightHand.transform.position)
	...
	if loadValue("NTS-Spawner-Mode") == 30 then
		game.spawnObjectNetwork("item_food_dragonfruit", game.playerController.rightHand.transform.position)
		game.spawnObjectNetwork("item_food_dragonfruit", game.playerController.rightHand.transform.position)
		game.spawnObjectNetwork("item_food_dragonfruit", game.playerController.rightHand.transform.position)
		game.spawnObjectNetwork("item_food_dragonfruit", game.playerController.rightHand.transform.position)
	...
	if loadValue("NTS-Spawner-Mode") == 31 then
		local enemy = game.spawnObjectNetwork("boss_king_slime", game.playerController.rightHand.transform.position)
		local gameObject = enemy.gameObject
		local enemyBase = getComponent(gameObject, "EnemyBase")
		enemyBase.MaxHealth = loadValue("skl-maxhealth") * 2
        enemyBase.Health = loadValue("skl-maxhealth") * 2
		return
	end

	if loadValue("NTS-Spawner-Mode") == 32 then
		local orb = game.SpawnObjectNetwork("item_upgrade_all", game.playerController.rightHand.transform.position)
		local itembase = getComponent(orb, "ItemUpgrade")
		itembase.forceSpawn = "erthu_chrwa_bloodshot_eye"
		return
	end

	if loadValue("NTS-Spawner-Mode") == 34 then
		local orb = game.SpawnObjectNetwork("item_upgrade_all", game.playerController.rightHand.transform.position)
		local itembase = getComponent(orb, "ItemUpgrade")
		itembase.forceSpawn = "erthu_chrwa_glass_cannon"
		return
	end

	if loadValue("NTS-Spawner-Mode") == 35 then
		local orb = game.SpawnObjectNetwork("item_upgrade_all", game.playerController.rightHand.transform.position)
		local itembase = getComponent(orb, "ItemUpgrade")
		itembase.forceSpawn = "erthu_chrwa_dead_weight"
		return
	end

	if loadValue("NTS-Spawner-Mode") == 36 then
		local orb = game.SpawnObjectNetwork("item_upgrade_all", game.playerController.rightHand.transform.position)
		local itembase = getComponent(orb, "ItemUpgrade")
		itembase.forceSpawn = "erthu_chrwa_pristine_fletching"
		return
	end

	if loadValue("NTS-Spawner-Mode") == 37 then
		local orb = game.SpawnObjectNetwork("item_upgrade_all", game.playerController.rightHand.transform.position)
		local itembase = getComponent(orb, "ItemUpgrade")
		itembase.forceSpawn = "erthu_chrwa_hummingbird_feather"
		return
	end
